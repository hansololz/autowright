...
describe('result files e2e', () => {
  ...
  it('renders a markdown result file on the execution page', async () => {
    ...
    // §7 WORKSPACE card sits at the page's bottom, collapsed by default:
    // open it, then check the reveal button is there — never clicked (it
    // would open a real file-manager window).
    const workspace = page.getByTestId('workspace-card')
    await workspace.getByRole('button', { name: 'WORKSPACE' }).click()
    await workspace.getByRole('button', { name: COPY.reveal }).waitFor()
