...
- Optional **Needs fixing banner** — shown first, whenever the §4.1 `problems` list is
  ...
  it clears by the automation running again, or by its triggers changing);
  `output-collapsed` → **Fix with AI** (the §7 button with the collapse seed: opens the
  §11 editor, appends the collapse system entry naming the latest execution's zero and
  the typical count, and sends the §11 canned collapse chat message carrying that
  execution's id, so the agent sees the empty run in full detail). The banner is pure §4.1 `problems`
...
any automation failed **or carries the §4.1 `overdue` or `output-collapsed` problem**
(deliberately those three only — the other `problems` kinds are config nits the in-app
amber chip covers; the dot is reserved for "something isn't working that should be", and
an automation that exits 0 over an empty input is exactly that) —
